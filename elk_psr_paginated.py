...
def get_data(psr_envi, api_key, tenant_id, start_date, end_date, elk_url):
    ...
    # https://psr-prodes.o9solutions.com/psr-transform/_search
...
#     rename_map = {
#         'name': 'envi_name',
#         'version': 'envi_version',
#         'hbase': 'timings_hbase',--
#         'live server': 'timings_liveserver',--
#         'total': 'timings_total',
#         'webapi': 'timings_webapi',
#         'redis': 'timings_redis',
#         'mssql': 'timings_mssql',
#         'mongodb': 'timings_mongodb'
#     }
#
...
def process_data(all_hits, df):
    OUTPUT_COLUMNS = [
        'envi_name', 'envi_version', 'timestamp', 'tenant_id',
        'tenant_name', 'id', 'request_type', 'trace_id',
        'workspace', 'page', 'view', 'report', 'action_button',
        'user_email', 'ui_origin', 'timings_webapi',
        'timings_redis', 'timings_mssql', 'timings_total',
    ...
    for hit in all_hits:
        ...
        # Detect v2: has 'service' key OR v2-style timing keys
        is_v2 = 'service' in source or 'o9_WebAPI' in timings or 'live-server' in timings

        if is_v2:
            http = request.get('http', {})  # nested http object in v2
            row = {
                'envi_name':        env.get('name'),
                'envi_version':     env.get('version'),
                'timestamp':        source.get('@timestamp'),
                'tenant_id':        request.get('tenant_id'),
                'tenant_name':      request.get('tenant_name'),
                'id':               request.get('id'),
                'request_type': f"{http.get('method', '')} {http.get('path', '')}".strip(),  # "POST /api/v2/widget/getmeta"
                'trace_id':         request.get('trace_id'),
                'workspace':        request.get('workspace'),
                'page':             request.get('page'),
                'view':             request.get('view'),
                'report':           request.get('report'),
                'action_button':    request.get('action_button'),
                'user_email':       request.get('user_email'),
                'ui_origin':        request.get('ui_origin'),
                'timings_webapi':   timings.get('o9_WebAPI'),       # renamed in v2
                'timings_redis':    timings.get('redis'),      # use .get() — may appear in some responses
                'timings_mssql':    timings.get('mssql'),
                'timings_total':    timings.get('total'),
                'timings_mongodb':  timings.get('mongodb'),
                'timings_hbase':    timings.get('hbase'),
                'timings_liveserver': timings.get('live-server'),   # renamed in v2
            }
        else:
            ...
            row = {
                ...
                'user_email':       request.get('user_email'),
                'ui_origin':        request.get('ui_origin'),
                'timings_webapi':   timings.get('webapi'),
                'timings_redis':    timings.get('redis'),
                'timings_mssql':    timings.get('mssql'),
                'timings_total':    timings.get('total'),
                'timings_mongodb':  timings.get('mongodb'),
                'timings_hbase':    timings.get('hbase'),
                'timings_liveserver': timings.get('live server'),
            }
...
def save_to_csv(df, psr_envi, tenant_id):
    ...
    df = df[
        (df['workspace'].notnull()) &
        (df['workspace'] != '') &
        (~df['user_email'].str.contains('o9solutions', na=False)) &
        (~df['report'].str.contains('View', na=False)) &
        (~df['user_email'].str.contains('etluser', na=False))
    ]
    print(len(df))
...
def main():
    try:
        ...
        print(f"Total rows retrieved: {len(response)}")
        if response:
            # print(response)
